...
RUN_LOGS = Path("/Volumes/orange-dev/extra/pdbms-scratch/run-logs")
...
def fig_llm_model_pareto():
    ...
    for i, (_lab, r, c) in enumerate(zip(labels, rtfs, coss, strict=True)):
        dominated = any(
            (ro["rtf"] < r and ro["cos_sim"] >= c) or (ro["rtf"] <= r and ro["cos_sim"] > c)
            for j, ro in enumerate(rows)
            if j != i
        )
        if not dominated:
            front.append(i)
    ...
    for i, (lab, r, c) in enumerate(zip(labels, rtfs, coss, strict=True)):
        if i in front:
            continue
        ax.scatter(r, c, s=110, color=MUTED, alpha=0.55, zorder=2, edgecolor="white", linewidth=1.2)
        ax.annotate(
            lab, (r, c), xytext=(7, -2), textcoords="offset points", fontsize=7.5, color=MUTED
        )
    ...
    ax.set_title(
        "LLM analyst — 7-model Pareto on a 16-minute team meeting\n"
...
def fig_whisper_prompt():
    ...
    ax.set_title(
        "Whisper bias prompt drops transcription error by 15-25 percentage points\n"
...
def fig_diar_embedding():
    ...
    axes[0].bar(backends, margins, color=colors, edgecolor="white", linewidth=0.8, zorder=3)
    ...
    axes[0].set_title(
        "Separability margin (inter − intra cosine)\nhigher = cleaner per-segment clustering",
        fontsize=9.5,
        loc="left",
    )
    ...
    axes[1].bar(backends, walls, color=colors, edgecolor="white", linewidth=0.8, zorder=3)
    ...
    axes[1].set_title(
        "Per-call wall time on Apple Silicon\n"
    ...
    fig.suptitle(
        "Speaker embedding backend — TitaNet separates 76 % better than pyannote\n"
...
def fig_llm_cadence():
    ...
    for lab, r, c in zip(s_labels, s_rtfs, s_coss, strict=True):
        ...
        axes[0].scatter(r, c, color=col, s=size, edgecolor="white", linewidth=1.4, zorder=3)
    ...
    for lab, r, c in zip(m_labels, m_rtfs, m_coss, strict=True):
        ...
        col = ACCENT_GREEN if is_winner else PRIMARY
        size = 130 if is_winner else 70
        axes[1].scatter(r, c, color=col, s=size, edgecolor="white", linewidth=1.4, zorder=3)
        prefix = "★ " if is_winner else ""
        axes[1].annotate(
            prefix + lab,
            (r, c),
    ...
    axes[1].set_title(
        "Median over four team meetings (16 to 33 min each)", fontsize=9.5, loc="left"
    )
    ...
    fig.suptitle(
        "Rolling-summary cadence — refresh every 60 s of content is the best trade-off\n"
