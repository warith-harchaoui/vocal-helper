...
def _parse_vtt_text(vtt_path: Path, max_seconds: float | None = None) -> str:
    ...
    for raw_line in vtt_path.read_text(encoding="utf-8").splitlines():
        ...
        m = ts_re.search(line)
        if m:
            cue_end = int(m.group(4)) * 3600 + int(m.group(5)) * 60 + int(m.group(6))
...
def clip_pcm(ph) -> tuple[np.ndarray, int]:
    ...
    """
    try:
        pcm, sample_rate = asyncio.run(_drain_with_timeout(VIDEO_URL, CLIP_S, INGEST_TIMEOUT_S))
...
def yt_subtitles(tmp_path_factory: pytest.TempPathFactory, yh) -> Path:
    ...
    pytest.skip(f"no captions retrievable for {VIDEO_URL} (last error: {last_err})")
...
def test_streaming_pipeline_yields_utterance(
    ...
    assert any(u["text"].strip() for u in utterances), "every utterance text was empty"
