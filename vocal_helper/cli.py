...
def _build_config(args: argparse.Namespace) -> PipelineConfig:
    asr_cfg: dict = {
        ...
        # ``getattr`` so tests that build a partial Namespace don't have
        # to know about every new lever we add.
    ...
    if getattr(args, "eot", False):
        # SemanticEOTStage is opt-in : enabling it activates the
        # LiveKit-style turn-detector that gates VAD-emitted segments
        # through a small LLM completeness classifier.
        eot_cfg = {}
...
def main() -> None:
    ...
    def add_common(sp: argparse.ArgumentParser) -> None:
        ...
        sp.add_argument(
            ...
            help="Whisper bias prompt — name the domain and a few "
                 "expected proper nouns. Cuts WER 15-25 pp + saves up "
                 "to 39%% RTF on AMI (2026-06-30 sweep). "
                 "Example: 'medical telemedicine consultation: patient "
                 "symptoms, medication review, follow-up appointment'.",
        )
        sp.add_argument(
            "--diar-backend", choices=["pyannote", "nemo"], default="nemo",
            help="Embedding backend for online diar. Default 'nemo' "
                 "(TitaNet) — +76%% separability margin over 'pyannote' "
                 "on AMI per the 2026-06-30 sweep, at 7x per-call "
                 "latency (45 ms vs 6 ms — still negligible per "
                 "voiced segment). Switch to 'pyannote' if the NeMo "
                 "install footprint is prohibitive.",
        )
        ...
        sp.add_argument(
            "--eot", action="store_true",
            help="enable the SemanticEOTStage (LiveKit-style turn "
                 "detector). Holds back VAD segments that look mid-thought, "
                 "merges them with their successor, reduces mid-sentence "
                 "cuts at the cost of one extra LLM hop per voiced segment.",
        )
        sp.add_argument(
            "--eot-model", default=None,
            help="Ollama model for the EOT classifier (default qwen2.5:3b).",
        )
