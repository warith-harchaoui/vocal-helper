...
async def _run_pipeline(args: argparse.Namespace, source_factory) -> None:
    ...
    if getattr(args, "offline", False):
        ...
        offline_cfg = OfflinePipelineConfig(
            diar=config.diar, asr=config.asr, llm=config.llm,
        )
...
def _handle_transcribe(args: argparse.Namespace) -> int:
    ...
    # Lazy imports so ``vocal-helper --help`` never pays the numpy / soundfile
    # / whisper.cpp cost for people who only wanted usage text.
    import numpy as np
    import soundfile as sf  # part of the [dev] extra, cheap on install

    from vocal_helper.asr import transcribe_pcm

    pcm, sr = sf.read(args.path, dtype="float32", always_2d=False)
    # Down-mix stereo to mono — pywhispercpp expects a single channel.
    if pcm.ndim == 2:
