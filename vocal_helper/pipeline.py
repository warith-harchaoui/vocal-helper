...
class Pipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self._q_pcm: asyncio.Queue[PcmFrame | None] = asyncio.Queue(
            maxsize=self.config.qsize_pcm
        )
        ...
        self._q_utt: asyncio.Queue[Utterance | None] = asyncio.Queue(
            maxsize=self.config.qsize_seg
        )
    ...
    def subscribe_voiced(
        self, cb: Callable[[VoicedSegment], Awaitable[None]]
    ) -> None:
        """Async callback for every :class:`VoicedSegment` after VAD."""
        self._voiced_subs.append(cb)

    def subscribe_diarized(
        self, cb: Callable[[DiarizedSegment], Awaitable[None]]
    ) -> None:
        """Async callback for every :class:`DiarizedSegment` after diarization."""
        self._diar_subs.append(cb)

    def subscribe_utterances(
        self, cb: Callable[[Utterance], Awaitable[None]]
    ) -> None:
    ...
    async def run(self) -> AsyncIterator[Utterance | SummarySnapshot]:
        ...
        tasks.append(asyncio.create_task(
            self._asr.run(q_diar_for_asr, self._q_utt), name="vh.asr",
        ))
        ...
        if self._llm is not None:
            tasks.append(asyncio.create_task(
                self._llm.run(q_utt_for_llm, self._q_summary), name="vh.llm",
            ))
        else:
            # Drain to /dev/null so back-pressure doesn't stall the tee.
            tasks.append(asyncio.create_task(
                self._drain(q_utt_for_llm), name="vh.llm.disabled",
            ))
...
class OfflinePipeline:
    ...
    async def run(self) -> AsyncIterator[Utterance | SummarySnapshot]:
        ...
        tasks.append(asyncio.create_task(
            self._diar.run(self._q_pcm, self._q_diar), name="vh.offline.diar",
        ))
        q_diar_for_asr: asyncio.Queue = asyncio.Queue(maxsize=self.config.qsize_seg)
        tasks.append(asyncio.create_task(
            self._tee(self._q_diar, q_diar_for_asr, self._diar_subs),
        ...
        tasks.append(asyncio.create_task(
            self._asr.run(q_diar_for_asr, self._q_utt), name="vh.offline.asr",
        ))
        q_utt_for_llm: asyncio.Queue = asyncio.Queue(maxsize=self.config.qsize_seg)
        q_utt_for_output: asyncio.Queue = asyncio.Queue(maxsize=self.config.qsize_seg)
        tasks.append(asyncio.create_task(
        ...
        if self._llm is not None:
            tasks.append(asyncio.create_task(
                self._llm.run(q_utt_for_llm, self._q_summary), name="vh.offline.llm",
            ))
        else:
            tasks.append(asyncio.create_task(
                self._drain(q_utt_for_llm), name="vh.offline.llm.disabled",
            ))
            await self._q_summary.put(None)
